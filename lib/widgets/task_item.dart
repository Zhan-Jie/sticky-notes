...
class _TaskItemState extends State<TaskItem> {
  ...
  Widget build(BuildContext context) {
    ...
    final isOvertime = task.focusState == FocusState.overtime;
                      ...
                      Padding(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 12,
                            color:
                                isOvertime ? Colors.redAccent : Colors.white54,
                          ),
                        ),
                      ),
                    if (showActions)
                        ...
                        child: Wrap(
                          ...
                          children: [
                            ...
                            _buildActionButton(
                              ...
                              onPressed: () => _requestDelete(context, task),
                            ),
                          ],
                        ),
                      ),
  ...
  String _buildStatusText(Task task) {
    ...
    if (task.focusState == FocusState.overtime) {
      return '超时 ${_formatDuration(task.overtimeSec)}';
    }
    if (task.focusState == FocusState.paused) {
      if (task.isOvertimePhase) {
        return '超时已暂停 ${_formatDuration(task.overtimeSec)}';
      }
